...
export function RescheduleDialog({ open, onOpenChange, doctorId, onReschedule, isPending }: RescheduleDialogProps) {
  const [date, setDate] = useState<Date | undefined>(undefined)
  const formattedDate = date ? format(date, 'yyyy-MM-dd') : ''
  const { data: availability, isLoading, isError, error } = useDoctorAvailability(doctorId, formattedDate)
        ...
        {date && (
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              {isError && (
                <span className="text-destructive">{(error as Error)?.message || 'Error loading availability'}</span>
              )}
              {!isLoading && !isError && availability && availability.slots.length === 0 && (
                ...
                availability.slots.map((slot) => {
                  ...
                  return (
                    <Button
                      ...
                      disabled={!slot.available || isPending || isPast}
                      type="button"
                      className="min-w-[90px]"
                      onClick={() => {
                        if (!slot.available || isPending || isPast) return
