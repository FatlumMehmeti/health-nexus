...
import { usersService, type UserRead } from "@/services/users.service";
import {
  patientsService,
...
function toPatientForm(profile: PatientProfileRead): PatientFormValues {
  return {
...
export function DashboardProfilePanel() {
  ...
  const [nexusInitial, setNexusInitial] = useState<NexusFormValues | null>(null);
  ...
  const [patientInitial, setPatientInitial] = useState<PatientFormValues | null>(null);
  ...
  useEffect(() => {
    if (!tenantMembershipsQuery.data || tenantMembershipsQuery.data.length === 0) return;
    if (!selectedTenantId) {
      setSelectedTenantId(String(tenantMembershipsQuery.data[0]!.tenant_id));
    }
  }, [tenantMembershipsQuery.data, selectedTenantId]);

  const patientProfileQuery = useQuery({
    queryKey: ["patient-profile", selectedTenantId],
    queryFn: () => patientsService.getMyTenantProfile(Number(selectedTenantId)),
  ...
  const savePatientMutation = useMutation({
    mutationFn: (payload: PatientProfileUpdate) =>
      patientsService.updateMyTenantProfile(Number(selectedTenantId), payload),
    onSuccess: (data) => {
  ...
  const onSaveNexus = () => {
    ...
    if (nexusForm.first_name !== nexusInitial.first_name) payload.first_name = nexusForm.first_name;
    if (nexusForm.last_name !== nexusInitial.last_name) payload.last_name = nexusForm.last_name;
    if (nexusForm.contact !== nexusInitial.contact) payload.contact = nexusForm.contact;
    if (nexusForm.address !== nexusInitial.address) payload.address = nexusForm.address;
  ...
  const patientNotRegistered =
    ...
    (patientProfileQuery.error.status === 403 || patientProfileQuery.error.status === 404);
  ...
  const genderChoices = patientForm.gender && !GENDER_OPTIONS.includes(patientForm.gender)
    ? [...GENDER_OPTIONS, patientForm.gender]
    : GENDER_OPTIONS;
  const bloodTypeChoices = patientForm.blood_type && !BLOOD_TYPE_OPTIONS.includes(patientForm.blood_type)
    ...
    <div className="space-y-4">
      <Tabs defaultValue="nexus-profile" className="space-y-4">
        <TabsList variant="line" className="grid w-full grid-cols-2">
          <TabsTrigger value="nexus-profile">Nexus Profile</TabsTrigger>
        ...
        <TabsContent value="nexus-profile" className="mt-0">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Nexus Profile</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setNexusForm((prev) => ({ ...prev, first_name: event.target.value }))
                    }
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setNexusForm((prev) => ({ ...prev, last_name: event.target.value }))
                    }
                    disabled={meQuery.isLoading}
                  />
                </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="profile-email" value={nexusForm.email} readOnly disabled />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setNexusForm((prev) => ({ ...prev, contact: event.target.value }))
                  }
                  disabled={meQuery.isLoading}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setNexusForm((prev) => ({ ...prev, address: event.target.value }))
                  }
                  disabled={meQuery.isLoading}
                />
              </div>
              ...
              {meQuery.isError ? (
                <p className="text-sm text-destructive">
                  {isApiError(meQuery.error) ? meQuery.error.displayMessage : "Failed to load profile."}
                </p>
              ) : null}
              ...
              <div className="flex justify-end">
                <Button
                  ...
                  disabled={!nexusDirty || meQuery.isLoading || saveNexusMutation.isPending}
                >
                  Save
                </Button>
              </div>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={selectedTenantId} onValueChange={setSelectedTenantId}>
                  ...
                  <SelectContent>
                    {(tenantMembershipsQuery.data ?? []).map((tenant) => (
              ...
              (tenantMembershipsQuery.data?.length ?? 0) === 0 ? (
                <p className="text-sm text-muted-foreground">
                  You do not have tenant memberships available for patient profile updates.
                </p>
              ) : null}
              ...
              {selectedTenantId && patientNotRegistered ? (
                <div className="space-y-3 rounded-md border p-3">
                  <p className="text-sm">You are not registered as a patient in this tenant.</p>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() =>
                      navigate({
                        ...
                        params: { tenantSlug: String(selectedTenant?.tenant_id ?? selectedTenantId) },
                      })
                    }
                  >
              ...
              {selectedTenantId && patientProfileQuery.isError && !patientNotRegistered ? (
                <p className="text-sm text-destructive">
                  {isApiError(patientProfileQuery.error)
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setPatientForm((prev) => ({
                          ...prev,
                          blood_type: value === NONE_VALUE ? "" : value,
                        }))
                      }
                    >
                  ...
                  <div className="flex justify-end">
                    <Button
                      ...
                      disabled={!patientDirty || savePatientMutation.isPending}
                    >
                      Save
                    </Button>
                  </div>
