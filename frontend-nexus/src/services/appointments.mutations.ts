...
export function useBookAppointment() {
  ...
  return useMutation({
    mutationFn: async (data: BookAppointmentRequest) => {
      const result = await apiFetch<BookAppointmentResponse>('/appointments/book', {
        method: 'POST',
        body: {
          tenant_id: parseInt(data.tenant_id),
          doctor_id: parseInt(data.doctor_id),
          department_id: data.department_id ? parseInt(data.department_id) : 1,
    ...
    onError: (error: any) => {
      const message = error?.detail || error?.message || 'Failed to book appointment'
      toast.error(message)
    },
  })
}
