...
jest.mock('@/services/enrollments.service', () => ({
  __esModule: true,
  useApprovedTenants: () => ({ data: [], isLoading: false, isError: false, error: null }),
  ApprovedTenant: {},
  EnrollmentDoctor: {},
}), { virtual: true })
  ...
  mockNotifications = [
    {
      ...
      message: 'A patient has requested an appointment on 2026-03-02.',
      is_read: false,
...
beforeEach(() => {
  ...
  globalThis.fetch = jest.fn(async (input: RequestInfo | URL) => {
    ...
    // Notifications list
    if (url.includes('/notifications/me') && !url.includes('unread-count') && !url.includes('read-all')) {
    ...
    if (url.includes('/notifications/me/unread-count')) {
      return new Response(JSON.stringify({ count: mockUnreadCount }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      })
    }
...
describe('FUL-29: Appointment routes exist in route tree', () => {
  it('route tree includes /appointments/book', () => {
    ...
    const routeIds = Object.keys((routeTree as any).routesByFullPath ?? (routeTree as any).children ?? {})
