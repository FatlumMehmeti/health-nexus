...
const ROUTES: Array<{ path: string; routeKey: RouteKey; expectHeading?: RegExp; expectText?: RegExp }> = [
  { path: '/dashboard', routeKey: 'DASHBOARD_HOME', expectText: /Health Nexus/i },
  { path: '/dashboard/data', routeKey: 'DASHBOARD_DATA', expectHeading: /Data Fetching/i },
  { path: '/dashboard/forms', routeKey: 'DASHBOARD_FORMS', expectHeading: /Form \+ Mutation/i },
]
...
function setMe401() {
  meStatus = 401
  setTokens({ accessToken: 'expired-token', refreshToken: 'test-refresh' })
}
...
describe('FUL-15 acceptance', () => {
  ...
  it('authenticated but forbidden role visiting route is redirected to /unauthorized', async () => {
    ...
    await act(async () => {
      await router.navigate({ to: '/dashboard/data' })
    })
    await waitFor(() => {
  ...
  it('authenticated allowed role sees route content', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByTestId('login-page')).not.toBeInTheDocument()
      expect(screen.getAllByText(/Health Nexus/i).length).toBeGreaterThan(0)
    })
  })
