...
function TestAppointmentDetail() {
  ...
  const [status, setStatus] = React.useState<'REQUESTED' | 'CONFIRMED' | 'CANCELLED' | 'COMPLETED'>('REQUESTED')
  ...
  const canReschedule = status === 'REQUESTED' || status === 'CONFIRMED'
  ...
  return (
    <div data-testid="appointment-detail">
      ...
      {/* Cancel Dialog */}
      {cancel && (
        <div data-testid="cancel-dialog" className="border p-4 rounded">
          <h2>Cancel Appointment</h2>
          <p>Are you sure you want to cancel this appointment? This action cannot be undone.</p>
          {cancelError && <div data-testid="cancel-error-message" className="text-red-600">{cancelError}</div>}
          <button
...
function TestRescheduleDialog({
  ...
  const handleSlotClick = async (slot: string) => {
    const isoDatetime = date ? `${date.toISOString().split('T')[0]}T${slot}:00` : ''
  ...
  return (
    <div data-testid="reschedule-dialog" className="border p-4 rounded">
      <h2>Reschedule Appointment</h2>
      <p>Select a new date and time slot for your appointment.</p>

      {error && <div data-testid="reschedule-error-message" className="text-red-600">{error}</div>}
      ...
      {date && (
        <div data-testid="slots-section">
          ...
          {availabilityLoading && <span data-testid="slots-loading">Loading...</span>}
          {!availabilityLoading && slots.length === 0 && <span data-testid="no-slots">No slots available for this date.</span>}
