...
describe('FUL-Signup form', () => {
  it('navigates to /login after successful signup (201)', async () => {
    global.fetch = jest.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes('/api/auth/signup')) {
        return new Response(
          JSON.stringify({ user_id: '99', email: 'new@test.com', role: 'client' }),
  ...
  it('shows inline error on 409 duplicate email without redirecting', async () => {
    ...
    await waitFor(() => {
      ...
      ).toMatch(/An account with this email already exists/i)
    })
  ...
  it('shows client-side validation error on password mismatch without calling fetch', async () => {
    ...
    const calls = (mockFetch.mock.calls as unknown as string[][]).flat()
    const signupCalls = calls.filter((url) => String(url).includes('/api/auth/signup'))
