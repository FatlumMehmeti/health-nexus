...
import * as tenantPlansService from "../services/tenant-plans.service";
...
function TestTenantPlansPanel() {
  ...
  const handleSubmit = () => {
    ...
    if (editingPlanId) {
      setPlans(plans.map((p) => (p.id === editingPlanId ? { ...payload, id: p.id } : p)));
    } else {
      setPlans([
        ...plans,
        { ...payload, id: Math.max(...plans.map((p) => p.id)) + 1 },
      ]);
    }
  ...
  return (
    <div data-testid="tenant-plans-panel">
      <h2>Manage plans</h2>
      <p>Add plans and toggle visibility. Changes are saved to the backend.</p>
      ...
      <div data-testid="plan-form" className="space-y-3">
        ...
        {/* Error message */}
        {submitError && (
          <div data-testid="submit-error" className="text-destructive text-sm">
            {submitError}
          </div>
        )}
      ...
      <div data-testid="plans-list" className="mt-6">
        <h3>Current Plans</h3>
        {plans.map((plan) => (
          <div key={plan.id} data-testid={`plan-row-${plan.id}`} className="border p-3 mb-2">
            <div className="flex justify-between items-center">
              <div>
                <span data-testid={`plan-name-${plan.id}`} className="font-semibold">
                  {plan.name}
                </span>
                <span data-testid={`plan-price-${plan.id}`} className="ml-4">
...
describe("FUL-75: TenantPlansPanel Management Screen", () => {
  ...
  describe("Real-time price input validation (FUL-75 requirement)", () => {
    ...
    it("reverts hint to muted color when price returns to valid range", async () => {
      ...
      await user.type(priceInput, "25");
      await waitFor(() => expect(hint).toHaveClass("text-destructive"));
  ...
  describe("Edit plan with pricing constraints", () => {
    it("loads plan data into form when edit is clicked", async () => {
      ...
      const priceInput = screen.getByTestId("plan-price-input") as HTMLInputElement;
    ...
    it("updates plan successfully when price is within bounds", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("plan-price-1")).toHaveTextContent("€175.99");
      });
    });
  });
  ...
  describe("Management screen features", () => {
    ...
    it("shows action buttons for each plan (edit, toggle)", () => {
      ...
      expect(screen.getByTestId("toggle-visibility-btn-1")).toBeInTheDocument();
      expect(screen.getByTestId("edit-btn-2")).toBeInTheDocument();
      expect(screen.getByTestId("toggle-visibility-btn-2")).toBeInTheDocument();
    });
  });
});
