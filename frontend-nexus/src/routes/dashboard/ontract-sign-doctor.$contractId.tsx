      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Only the assigned doctor can sign this contract.
        ...
        <CardHeader>
          ...
          <CardDescription>
            <span className="flex flex-wrap items-center gap-2">
              Doctor: {contract.doctor_name ?? "Assigned doctor"} · Tenant:{" "}
              {contract.tenant_name ?? `ID ${contract.tenant_id}`} · Status:{" "}
              <Badge variant={statusBadgeVariant(contract.status)}>
                {contract.status}
              </Badge>
              · Salary: {formatEuro(contract.salary)} · {formatDate(contract.start_date)} – {formatDate(contract.end_date)}
            </span>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-lg border p-3">
              ...
              <p className="font-medium">{contract.doctor_name ?? "-"}</p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground">Hospital Signature</p>
              <p className="font-medium">
                {contract.hospital_signed_at ? formatDate(contract.hospital_signed_at) : "Not signed yet"}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground">Doctor Signature</p>
              <p className="font-medium">
                {contract.doctor_signed_at ? formatDate(contract.doctor_signed_at) : "Pending your signature"}
              </p>
            </div>
          </div>
            ...
            <div className="rounded-lg border bg-muted/20 p-3">
              {isPdfLoading ? (
                ...
                <p className="text-sm text-muted-foreground">
                  PDF preview unavailable right now. Please try refreshing this page.
                </p>
              )}
            </div>
          </div>
          ...
          {contract.doctor_signed_at ? (
            <div className="rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-900 dark:bg-green-950/30">
              <p className="font-medium flex items-center gap-2 text-green-800 dark:text-green-200">
                <CheckIcon className="size-4" /> You have signed this contract on {formatDate(contract.doctor_signed_at)}.
              </p>
            </div>
          ) : canSign ? (
