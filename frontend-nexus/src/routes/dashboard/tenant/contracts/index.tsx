...
import {
  ContractDialog,
  type ContractDialogHandle,
} from "./components/ContractDialog";
import {
  ContractPdfDocument,
  type ReactPdfPrimitives,
} from "./components/ContractPdfDocument";
...
export function ContractsPage({
  ...
  const contractDialogRef = React.useRef<ContractDialogHandle>(null);
  ...
  const handleCreateClick = () => {
    contractDialogRef.current?.openCreate();
  };

  const handleEditClick = (contract: Contract) => {
    contractDialogRef.current?.openEdit(contract);
  };
  ...
  const handleDownloadPdf = async (contract: Contract) => {
    try {
      ...
      downloadBlob(
        blob,
        `contract_${contract.id}_doctor_${contract.doctor_user_id}.pdf`,
      );
      toast.success(
        `Downloaded contract_${contract.id}_doctor_${contract.doctor_user_id}.pdf`,
      );
    } catch (error) {
    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Eligible contracts must be ACTIVE, have doctor + hospital
            signatures, and be within the configured start/end date range.
          </CardDescription>
        </CardHeader>
      </Card>
