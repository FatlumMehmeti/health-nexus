...
import type { ApprovedTenant, EnrollmentDoctor } from '@/services/enrollments.service'
...
function SelectTenantPage() {
  ...
  const { data: tenants, isLoading, isError, error } = useApprovedTenants()
  const [selectedTenant, setSelectedTenant] = useState<ApprovedTenant | null>(null)

  const handleTenantSelect = (tenant: ApprovedTenant) => {
    setSelectedTenant(tenant)
  }

  const handleDoctorSelect = (tenant: ApprovedTenant, doctor: EnrollmentDoctor) => {
    useAuthStore.setState({ tenantId: String(tenant.id) })
    navigate({
      to: '/appointments/book',
      search: { doctorId: String(doctor.user_id) },
    })
  }
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {isError && (
          <div className="rounded-lg border border-destructive/20 bg-destructive/10 p-6">
            <p className="text-sm font-medium text-destructive">Failed to load clinics</p>
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {tenant.moto && (
                        <p className="text-xs text-muted-foreground mt-0.5 truncate">{tenant.moto}</p>
                      )}
                    </div>
                  </div>
                </CardHeader>
            ...
            {selectedTenant.doctors.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground max-w-md">
                  This clinic doesn't have any doctors available at the moment. Please try another clinic.
                </p>
              ...
              <div className="flex flex-col gap-3">
                {selectedTenant.doctors.map((doctor) => (
                  <Card
                    key={doctor.user_id}
                    className="cursor-pointer transition-all hover:border-primary/50 hover:shadow-md hover:shadow-primary/5"
                    onClick={() => handleDoctorSelect(selectedTenant, doctor)}
                  >
