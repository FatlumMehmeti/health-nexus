...
from app.services.payment_service import _mark_payment_as_captured
...
def test_captured_subscription_payment_stays_pending_until_superadmin_approval(
    client,
    bluestone_auth,
):
    """Captured payment should not activate a tenant subscription until a super admin approves it."""
    ...
    try:
        ...
        active_subscription_id = current_subscription.id
    ...
    try:
        payment = Payment(
            ...
            price=float(higher_plan.price),
            tenant_id=tenant_id,
            reference_id=pending_subscription_id,
            reference_type="tenant_subscription",
            idempotency_key="test-pending-subscription-capture",
            status=PaymentStatus.INITIATED,
            stripe_payment_intent_id="pi_test_pending_subscription_capture",
        )
        session.add(payment)
        session.commit()
        session.refresh(payment)

        _mark_payment_as_captured(session, payment)

        pending_subscription = session.get(TenantSubscription, pending_subscription_id)
        active_subscription = session.get(TenantSubscription, active_subscription_id)

        assert payment.status == PaymentStatus.CAPTURED
        assert pending_subscription is not None
        assert pending_subscription.status == SubscriptionStatus.EXPIRED
        assert pending_subscription.activated_at is None
        assert pending_subscription.expires_at is None
    ...
    request_resp = client.get(
        f"/api/subscription_plan/request/{pending_subscription_id}",
        headers=manager_headers,
    )
    assert request_resp.status_code == 200
    assert request_resp.json()["admin_status"] == "PENDING"
    assert request_resp.json()["latest_payment_status"] == "CAPTURED"


def test_superadmin_can_approve_subscription_after_payment_capture(
    client,
    bluestone_auth,
    superadmin_auth,
):
    """Super admins should see a captured request as pending and activate it explicitly."""
    ...
    try:
        ...
        assert higher_plan is not None
        previous_active_subscription_id = current_subscription.id
        tenant_id = bluestone.id
    finally:
        session.close()
    ...
    try:
        payment = Payment(
            ...
            reference_id=pending_subscription_id,
            reference_type="tenant_subscription",
            idempotency_key="test-superadmin-subscription-approval",
            status=PaymentStatus.INITIATED,
            stripe_payment_intent_id="pi_test_superadmin_subscription_approval",
        )
        session.add(payment)
        session.commit()
        session.refresh(payment)
    ...
    list_resp = client.get(
        "/api/superadmin/subscriptions?status_filter=PENDING",
        headers=admin_headers,
    )
    assert list_resp.status_code == 200
    pending_requests = list_resp.json()
    matching_request = next(
        request for request in pending_requests if request["id"] == pending_subscription_id
    )
    assert matching_request["latest_payment_status"] == "CAPTURED"
    assert matching_request["admin_status"] == "PENDING"
    ...
    assert transition_resp.json()["admin_status"] == "ACTIVE"
    assert transition_resp.json()["status"] == "ACTIVE"
    assert transition_resp.json()["latest_payment_status"] == "CAPTURED"

    session = SessionLocal()
    try:
        approved_subscription = session.get(TenantSubscription, pending_subscription_id)
        previous_active_subscription = session.get(
            TenantSubscription,
            previous_active_subscription_id,
        )

        assert approved_subscription is not None
        assert approved_subscription.status == SubscriptionStatus.ACTIVE
        assert approved_subscription.activated_at is not None
        assert approved_subscription.expires_at is not None

        assert previous_active_subscription is not None
        assert previous_active_subscription.status == SubscriptionStatus.EXPIRED
        assert previous_active_subscription.cancelled_at is not None
    finally:
        session.rollback()
        session.close()
